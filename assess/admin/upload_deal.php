<?php 
	...
	if($_FILES['myfile']['error']>0){
		...
		$sql="insert into tb_assess(filename,type,size,path,poster,posttime,kind,type2) values('$filename','$type','$size','$path','$poster','$p_time','$kind','$type2')";
		
		$query=mysqli_query($conn,$sql);
		if($query){
			mysqli_close($conn);
			echo "<script language='javascript' type='text/javascript'>alert('添加文件成功！');window.location.href=document.referrer;</script>";
		}
		else{
			mysqli_close($conn);
